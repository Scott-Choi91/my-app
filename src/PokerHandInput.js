...
export default function PokerHandInput() {
  ...
  // 배포한 URL을 아래 문자열로 교체하세요.
  const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwnHEKKlILnCPlfKxm6qFYUmI8SA7j8LATzW3_jAI9blFTk7_rxGiL3N37e-sHFM6QP/exec";
...
const handleSave = async () => {
  if (!SCRIPT_URL) {
    alert("SCRIPT_URL 이 비어있습니다. 배포한 Apps Script URL을 넣어주세요.");
    return;
  }
  ...
  useEffect(() => {
    // sumAmt 테스트
    const t1 = sumAmt([{ amount: 100 }, { amount: 200 }, { amount: "" }]) === 300;
    // preflopPotBase 테스트 (별도 함수로 동일 로직 재현)
    const simulatePreBase = (sbV, bbV, anteV, actions) => {
      let base = sbV + bbV + anteV;
      const sbIn = actions.some((a) => a.pos === "SB" && a.action !== "FOLD" && a.action !== "");
      const bbIn = actions.some((a) => a.pos === "BB" && a.action !== "FOLD" && a.action !== "");
      if (sbIn) base -= sbV;
      if (bbIn) base -= bbV;
      return base;
    };
    const t2 = simulatePreBase(100, 200, 100, [{ pos: "SB", action: "CALL" }]) === 300;
    const t3 = simulatePreBase(100, 200, 100, [{ pos: "BB", action: "CALL" }]) === 200; // 100+200+100-200

    console.log("[self-test] sumAmt:", t1, "preBase-SB in:", t2, "preBase-BB in:", t3);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="grid grid-cols-5 gap-4">
          <div>
            ...
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-3 py-2 rounded bg-slate-800" />
          </div>
          <div>
            <label className="block mb-1">타입</label>
            <select value={gameType} onChange={(e) => setGameType(e.target.value)} className="w-full px-3 py-2 rounded bg-slate-800">
          ...
          <div>
            ...
            <select value={speed} onChange={(e) => setSpeed(e.target.value)} className="w-full px-3 py-2 rounded bg-slate-800">
          ...
          <div>
            ...
            <input value={venue} onChange={(e) => setVenue(e.target.value)} className="w-full px-3 py-2 rounded bg-slate-800" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select value={heroPosition} onChange={(e) => setHeroPosition(e.target.value)} className="w-full px-3 py-2 rounded bg-slate-800">
              <option value="">-</option>
      ...
      {[
        ...
        <div key={st.name} className="border-t border-slate-700 pt-4">
            ...
            {st.actions.map((a, i) => (
              <div key={i} className="grid grid-cols-4 gap-2 items-center">
                ...
                >
                  ...
                  {(st.name === "프리플랍"
                    ? preflopPositions
                    : postflopPositions.filter((p) => survivorsFrom(idx === 1 ? preflopActions : idx === 2 ? flopActions : turnActions).includes(p))
                  ).map((p) => (
                    <option key={p} value={p}>
                      {getPositionLabel(p)}
                    </option>
